...
export class CopyDocs {
  ...
  public generate(paths: string[]): void {
    paths.forEach((path) => {
      ...
      if (source) {
        ...
        if (subComponents) {
          subComponents.forEach((docSubComponent) => {
            const newDoc = {
  ...
  private createDoc(doc: any): void {
    ...
    const dist = `${basePath}/${dotCase(doc.id)}.docs.json`;
    writeFileSync(dist, JSON.stringify(doc, null, 2));
    console.log(`created ./dist/elements/${dotCase(doc.id)}.docs.json ✅`);
  }
...
const paths = glob.sync(
  nodepath.join(
    __dirname,
    "../../implementations/react/src/*/*/src/*.docs.json",
  ),
);
